...
<?php
	...
	$query1=mysqli_query($con,"SELECT * FROM sbmp_subjects WHERE semester='3'");
...
for($x=0; $x<6; $x++)
{
if (($sem3[$x]['subject']!=$backlog1) && ($sem3[$x]['subject']!=$backlog2) && ($sem3[$x]['subject']!=$backlog3) && ($sem3[$x]['subject']!=$backlog4) && ($sem3[$x]['subject']!=$unchecked1) && ($sem3[$x]['subject']!=$unchecked2) && ($sem3[$x]['subject']!=$unchecked3) && ($sem3[$x]['subject']!=$unchecked4))
{
    echo '<table class="a" align="center" border="1">';
	echo '<tr>';
	echo '<td>';
	...
	echo '<td>';
	echo '<h4>';
    echo $sem3[$x]['subject_code'];
	echo '</h4>';
	echo '</td>';
	echo '<td>';
	echo '<h4>';
    echo $sem3[$x]['semester'];
	echo '</h4>';
	echo '</td>';
	echo '<td>';
	echo '<h4>';
...
if($stusemester<3)
{
	echo "<script type='text/javascript'>alert('You have not appeared for the 3rd semester yet!!!')</script>";
}
else
{
}
