...
<?php
	...
	$query1=mysqli_query($con,"SELECT * FROM sbmp_subjects WHERE semester='4'");
...
echo '&nbsp';
echo '&nbsp';
echo '<a href="http://localhost:81/SOPHOMORE/studentprofilesem3.php">';
...
for($x=0; $x<6; $x++)
{
if (($sem4[$x]['subject']!=$backlog1) && ($sem4[$x]['subject']!=$backlog2) && ($sem4[$x]['subject']!=$backlog3) && ($sem4[$x]['subject']!=$backlog4) && ($sem4[$x]['subject']!=$unchecked1) && ($sem4[$x]['subject']!=$unchecked2) && ($sem4[$x]['subject']!=$unchecked3) && ($sem4[$x]['subject']!=$unchecked4))
{
    echo '<table class="a" align="center" border="1">';
	echo '<tr>';
	echo '<td>';
	...
	echo '<td>';
	echo '<h4>';
    echo $sem4[$x]['subject_code'];
	echo '</h4>';
	echo '</td>';
	echo '<td>';
	echo '<h4>';
    echo $sem4[$x]['semester'];
	echo '</h4>';
	echo '</td>';
	echo '<td>';
	echo '<h4>';
...
if($stusemester<4)
{
	echo "<script type='text/javascript'>alert('You have not appeared for the 4th semester yet!!!')</script>";
}
else
{
}
